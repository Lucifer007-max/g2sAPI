...
async function authenticateAdmin({ email, password }) {
    const user = await db.G2Sadmin.scope('withHash').findOne({ where: { email:email}});
...
async function authenticate({ email, password }) {
    const user = await db.G2Sadmin.scope('withHash').findOne({ where: { email:email,role:'user' } });
...
async function getAll( page=1,limit=10,approved="") {
    ...
    let count = await db.G2Sadmin.count({ where: filterBy });
    ...
    let new_users = await db.G2Sadmin.count({ where: filterBy , approved })
    let res = {
        pagination:{
            total_count:count,
    ...
    // let award  = await db.G2SfileUploads.findOne({ where: {id: data.award_honor_id } }); 
    // let usser = data.get();
    ...
    // let publications  = await db.G2SfileUploads.findOne({ where: {id: data.publication_id } }); 
    // data['publicationsfile'] = publications.get().filePath;

    // let Achievement   = await db.G2SfileUploads.findOne({ where: {id: data.achievement_in_transfusion_medicine } }); 
    // data['Achievementfile'] = Achievement.get().filePath;

    // let ScientificAssociationTransfusion    = await db.G2SfileUploads.findOne({ where: {id: data.membership_of_association } }); 
...
async function approvedLists(page=1, limit=10, approved="2"){
    ...
    let count = await db.G2Sadmin.count({ where: approved});
...
async function reject(id){
    ...
    let image  = await db.G2SfileUploads.findOne({ where: {id: data.publication_id}}); 
    ...
    let image1  = await db.G2SfileUploads.findOne({ where: {id: data.award_honor_id } }); 
    data['award_honor_id_image'] = image1.get().filePath;

    let image2  =  await db.G2SfileUploads.findOne({ where: {id: data.achievement_in_transfusion_medicine } }); 
    data['achievement_in_transfusion_medicine_image']  = image2.get().filePath;

    let image3  =  await db.G2SfileUploads.findOne({ where: {id: data.membership_of_association } }); 
...
async function create(params) {
    // validate
    if (await db.G2Sadmin.findOne({ where: { email: params.email } })) {
...
//     params['approved'] = 0;
//     //Create user
//     let new_user = await db.G2Sadmin.create(params);
...
async function getUser(id) {
    var user = await db.G2Sadmin.findByPk(id);
    if (!user) throw 'User not found';
    
    let image  = await db.G2SfileUploads.findOne({ where: {id: user.user_img_id } }); 
    let usser = user.get();
    if('filePath' in usser){
...
async function getUserProfile(id) {
    ...
    // const countryName  = await db.G2Sadmin.findOne({ where: { id: user.country_id } }); 
...
async function approveUser(userId , params){
    let user = await db.G2Sadmin.findOne({ where: { id: userId, role:'user' } });
    ...
    let res = await db.G2Sadmin.update(
        { approved: 2 }, 
        {where: { id: userId  }
    });
...
async function updatePassword(id,data){
    ...
    const user = await db.G2Sadmin.scope('withHash').findOne({ where: { id:id,role:'user' } });
...
async function AdminupdatePassword(id,data){
    const user = await db.G2Sadmin.scope('withHash').findOne({ where: { id:id,role:'G2Sadmin' } });
...
async function getroledata(id){
    const list = await db.G2Sadmin.findByPk(id);
