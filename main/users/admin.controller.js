...
function register(req, res, next) {
    ...
    // adminService.create(req.body , req.params.id)
    .then((data) => res.json(data))
    .catch(next);
}
  
function updatePassword(req,res,next){
    adminService.updatePassword(req.params.id,req.body)
    .then((data) => res.json(data))
    .catch(next);
}

function getAll(req, res, next) {
    adminService.getAll(req.query.page,req.query.limit,req.query.approved)
        .then(users => res.json(users))
        .catch(next);
}

function getApproved(req , res, next) {
    adminService.approvedLists(req.query.page,req.query.limit,req.query.approved)
    .then(lists => res.json(lists))
    .catch(next);
}

function getReject(req , res, next) {
    adminService.getRejectLists(req.query.page,req.query.limit,req.query.approved)
    .then(lists => res.json(lists))
    .catch(next);
}
...
function getUserProfile(req, res, next) {
    adminService.getUserProfile(req.params.id)
        .then(user => res.json(user))
        .catch(next);
}

function getallapproveusers(req, res, next) {
    adminService.getallapproveusers(req.params.id)
        .then(user => res.json(user))
        .catch(next);
}
...
function userProfileUpdate(req, res, next ) {
    adminService.userProfileUpdate(req.params.id , req.body)
    .then(user => res.json(user))
    .catch(next);
}
...
function paymentData(req, res, next) {
    adminService.getPaymentDetails(req.body)
        .then(data => res.json(data))
        .catch(next);
}
function updatePaymentDetails(req, res, next) {
    adminService.updatePaymentDetails(req.body)
        .then(data => res.json(data))
        .catch(next);
}
...
function approveMail(req, res, next) {
    adminService.approveMail(req.body)
    .then(lists => res.json(lists))
    .catch(next)
}

function rejectUser(req, res, next) {
    adminService.rejectUser(req.params.id,req.body.value)
    .then(() => res.json({msg : 'User reject Sucessfully'} ))
    .catch(next);
}

function rejectMailservice(req, res, next) {
    adminService.rejectMail(req.body)
    .then(lists => res.json(lists))
    .catch(next)
}

function getNotifications(req,res,next){
    adminService.getNotifications(req.user.id)
    .then((list) => res.json(list))
    .catch(next);
}

function removeNotify(req, res, next) {
    adminService.removeNotify(req.params.id)
    .then(() => res.json({msg : 'Marked as read'}))
    .catch(next)
}

function rejectListByID (req , res , next) {
    adminService.rejectListsById(req.params.id)
    .then((list) => res.json(list))
    .catch(next);
}
...
function Rejectupdate(req, res, next) {
    adminService.Rejectupdate(req.params.id, req.body)
...
function getnById (req , res , next) {
    adminService.getNotificationsById(req.params.id)
    .then((list) => res.json(list))
    .catch(next);
}
...
function userimageUpdate(req, res, next ) {
    adminService.userimageUpdate(req.params.id , req.body)
    .then(() => res.json({msg : 'Image Upload Sucessfully'}))
    .catch(next);
}

function getBycountry(req , res, next){
    adminService.getBycountry(req.params.id)
    .then((list) => res.json(list))
    .catch(next);
}
function approveUserBycountry(req, res, next) {
    adminService.approveUserBycountry(req.params.id)
        .then(data => res.json(data))
        .catch(next);
}
function rejectUserBycountry(req, res, next) {
    adminService.rejectUserBycountry(req.params.id)
        .then(data => res.json(data))
        .catch(next);
}
...
function getTransitionstatus(req, res, next) {
    adminService.getTransitionstatus(req.params.id)
        .then(user => res.json(user))
        .catch(next);
}
...
function roleupdate(req,res,next){
    adminService.roleupdate(req.params.id,req.body)
